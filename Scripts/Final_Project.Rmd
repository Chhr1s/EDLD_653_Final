...
groups <- 
  c('ALL Students', 
    ...
    #'Kindergarteners' # add this to trigger warning
  )

schools_of_interest <- c(
...
```{r}
select_group <- function(cleaned_data_frame, test_group){
...
# Failed
#dat_vector <- vector(mode = "list",length = length(schools_of_interest))
# for (i in 1:3){
#   dat_vector[i] <- filter(dat, instn_name == schools_of_interest[i])
# }
# dat_vector
# dat_school <- filter(dat,instn_name == 'Bacon County High School')
...
p <-
  ...
  mutate(
    ...
    title = glue::glue('Percent Graduation of Student Groups for
